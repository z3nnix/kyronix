...
static bool port_identify(int idx) {
    ...
    ap->disk_sectors = (uint64_t)id[100]
                     | ((uint64_t)id[101] << 16)
                     | ((uint64_t)id[102] << 32)
                     | ((uint64_t)id[103] << 48);
    if (!ap->disk_sectors)
        ap->disk_sectors = (uint64_t)id[60] | ((uint64_t)id[61] << 16);
    ...
    ap->disk_model[40] = '\0';
    for (int i = 39; i >= 0 && ap->disk_model[i] == ' '; i--)
        ap->disk_model[i] = '\0';
...
static bool port_init(int idx) {
    ...
    if (port_identify(idx)) {
        ...
        log_info("AHCI: port %d  \"%s\"  %lu sectors  (%lu MiB)",
                 idx, ap->disk_model, ap->disk_sectors, mib);
    } else {
...
bool ahci_init(void) {
    ...
    for (int i = 0; i < AHCI_MMIO_PAGES; i++) {
        int r = vmm_map(&g_kernel_space,
                        AHCI_MMIO_VBASE + (uint64_t)i * PAGE_SIZE,
...
int ahci_flush(int port) {
    ...
    while (timeout--) {
        if (!(p->ci & 1u)) break;
        if (p->is & PORT_IS_FATAL) { port_comreset(p); return -1; }
        cpu_relax();
    }
    if (p->ci & 1u) {
