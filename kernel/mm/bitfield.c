...
void field_init(bitfield_t *self) {
    for (uint64_t i = 0; i < FIELD_N; ++i) { self->rows[i] = 0; }
}
...
llfree_result_t field_set_next(bitfield_t *field, frame_id_t start_frame, size_t order) {
    ...
    if (num_frames <= LLFREE_ATOMIC_SIZE) {
        for_offsetted(row, FIELD_N, current_i) {
            size_t pos = 0;
            uint64_t old;
            if (atom_update(&field->rows[current_i], old, first_zeros_aligned, order, &pos)) {
                return llfree_ok(frame_id((current_i * LLFREE_ATOMIC_SIZE) + pos), 0);
            }
        }
    ...
    for_offsetted(row / entries, FIELD_N / entries, current_i) {
        ...
        for (size_t i = 0; i < entries; i++) {
            ...
            uint64_t old = 0;
            if (atom_cmp_exchange(&field->rows[idx], &old, UINT64_MAX)) { continue; }

            // Undo changes
            for (size_t j = 0; j < i; j++) {
                size_t idx = (current_i * entries) + j;
                uint64_t old = UINT64_MAX;
                if (!atom_cmp_exchange(&field->rows[idx], &old, 0)) {
        ...
        if (!failed) {
            ...
            return llfree_ok(frame_id(current_i * entries * LLFREE_ATOMIC_SIZE), 0);
        }
    }
...
llfree_result_t field_toggle(bitfield_t *field, size_t index, size_t order, bool expected) {
    ...
    if (num_frames > LLFREE_ATOMIC_SIZE) {
        ...
        for (size_t i = 0; i < num_entries; i++) {
            ...
            uint64_t old = mask;
            if (atom_cmp_exchange(&field->rows[pos.row + i], &old, ~mask)) { continue; }

            // undo
            for (size_t j = 0; j < i; j++) {
                bool success = atom_cmp_exchange(&field->rows[pos.row + i - j - 1], &mask, ~mask);
                assert(success);
            }
            return llfree_err(LLFREE_ERR_MEMORY);
        }
