...
static size_t child_count(const lower_t *self) { return div_ceil(self->frames, CHILD_N); }
...
static void zero_field(bitfield_t *field) {
    for (size_t i = 0; i < FIELD_N; ++i) { *((uint64_t *) &field->rows[i]) = 0; }
}
...
llfree_result_t lower_init(lower_t *self, size_t frames, uint8_t init, uint8_t *primary) {
    ...
    assert((size_t) (self->children + tree_count) <= (size_t) (primary + meta));
...
uint8_t *lower_metadata(const lower_t *self) { return (uint8_t *) self->fields; }
...
static bool try_update_huge_n(lower_t *self, size_t base_idx, size_t h_num, child_t expected,
                              child_t desired) {
    for (size_t j = 0; j < h_num; j++) {
        child_t old = expected;
        if (!atom_cmp_exchange(get_child(self, base_idx + j), &old, desired)) {
            // Undo previous CAS operations
            for (size_t k = 0; k < j; k++) {
                child_t undo_old = desired;
                if (!atom_cmp_exchange(get_child(self, base_idx + (j - k)), &undo_old, expected)) {
...
static llfree_result_t lower_get_at(lower_t *self, frame_id_t frame, size_t order) {
    if (frame.value + (1u << order) > self->frames || frame.value % (1u << order) != 0) {
    ...
    if (order >= LLFREE_HUGE_ORDER) {
        ...
        if (try_update_huge_n(self, child_idx, h_num, child_new(LLFREE_CHILD_SIZE, false),
                              child_new(0, true)))
    ...
    if (atom_update(child, old, child_dec, order)) {
        ...
        llfree_result_t ret = field_toggle(field, frame.value % CHILD_N, order, false);
        if (llfree_is_ok(ret)) return llfree_ok(frame, 0);
...
llfree_result_t lower_get(lower_t *self, const frame_id_t start_frame, size_t order,
    ...
    if (!unlikely(frame.present)) {
        ...
        if (order >= LLFREE_HUGE_ORDER) {
            ...
            for_offsetted(idx / h_num, LLFREE_TREE_CHILDREN / h_num, current_i) {
                size_t i = current_i * h_num;
                if (try_update_huge_n(self, i, h_num, child_new(LLFREE_CHILD_SIZE, false),
                                      child_new(0, true))) {
                    return llfree_ok(frame_from_child(huge_id(i)), 0);
                }
            }
        ...
        for_offsetted(idx, LLFREE_TREE_CHILDREN, current_i) {
            ...
            if (atom_update(child, old, child_dec, order)) {
                llfree_result_t pos = field_set_next(&self->fields[current_i], start_frame, order);
                if (llfree_is_ok(pos)) {
                    frame_id_t offset = frame_from_child(huge_id(current_i));
                    return llfree_ok(frame_id(offset.value + pos.frame.value), 0);
                }
...
static llfree_result_t split_huge(child_t old, _Atomic(child_t) *child, bitfield_t *field) {
    llfree_result_t res = field_toggle(field, 0, LLFREE_CHILD_ORDER, false);
    if (llfree_is_ok(res)) {
        llfree_debug("split huge");
        bool success = atom_cmp_exchange(child, &old, child_new(0, false));
        assert(success);
    } else {
        llfree_debug("split huge: wait");
        for (size_t i = 0; i < RETRIES; i++) {
            child_t c = atom_load(child);
            if (!c.huge) return llfree_err(LLFREE_ERR_OK);
        }
...
llfree_result_t lower_put(lower_t *self, frame_id_t frame, size_t order) {
    ...
    if (frame.value + (1 << order) > self->frames || frame.value % (1 << order) != 0) {
        llfree_warn("invalid frame %" PRIu64 "\n", frame.value);
        return llfree_err(LLFREE_ERR_ARGUMENT);
    }
    ...
    if (order >= LLFREE_HUGE_ORDER) {
        ...
        if (child_idx != base_idx) { return llfree_err(LLFREE_ERR_ARGUMENT); }
    ...
    if (!llfree_is_ok(ret)) return ret;
...
ll_stats_t lower_stats(const lower_t *self) {
    ...
    for (size_t i = 0; i < children_count(self); ++i) {
        ...
        for (size_t j = 0; j < LLFREE_TREE_CHILDREN; ++j) {
            child_t child = atom_load(&self->children[i].entries[j]);
...
ll_stats_t lower_stats_at(const lower_t *self, frame_id_t frame, size_t order) {
    ...
    if (order == 0) {
        ...
        if (child.free == CHILD_N) {
            ...
            stats.free_frames = field_is_free(&self->fields[i], frame.value % CHILD_N);
            stats.free_huge = 0;
        }
        ...
        size_t start = align_down(frame.value >> LLFREE_CHILD_ORDER, LLFREE_TREE_CHILDREN);
...
void lower_print(const lower_t *self) {
    ...
    for (size_t i = 0; i < align_up(child_count(self), LLFREE_TREE_CHILDREN); i++) {
        if (i % LLFREE_TREE_CHILDREN == 0) llfree_info_cont("\n");
