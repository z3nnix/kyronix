...
#define for_offsetted(start, len, current_i)                                                       \
    for (size_t _i = 0, _offset = (start) % (len), _base_idx = (start) - _offset,                  \
                (current_i) = (start);                                                             \
         _i < (len); _i = _i + 1, (current_i) = _base_idx + ((_i + _offset) % (len)))
...
static inline ll_unused huge_id_t huge_id(size_t value) { return (huge_id_t) { .value = value }; }
...
static inline ll_unused row_id_t row_id(uint64_t value) { return (row_id_t) { .value = value }; }
...
static inline ll_unused size_t div_ceil(uint64_t a, size_t b) { return (a + b - 1) / b; }
...
static inline ll_unused size_t trailing_zeros(uint64_t val) {
    if (val == 0) return 8 * sizeof(uint64_t);
...
static inline ll_unused const char *INDENT(size_t indent) {
    ...
    const char *spaces = "                                ";
    if (indent >= max_indent / 4) return spaces;
