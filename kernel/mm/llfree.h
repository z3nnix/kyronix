...
#define ll_def_optional(ty, prefix, def)                                                           \
    ...
    static inline ll_unused prefix##_optional_t prefix##_some(ty value) {                          \
        return (prefix##_optional_t) { .present = true, .value = value };                          \
    }                                                                                              \
    static inline ll_unused prefix##_optional_t prefix##_none(void) {                              \
        return (prefix##_optional_t) { .present = false, .value = def };                           \
    }
...
static inline ll_unused tree_id_t tree_id(size_t value) { return (tree_id_t) { .value = value }; }
...
static inline llfree_result_t ll_unused llfree_ok(frame_id_t frame, uint8_t class) {
    return (llfree_result_t) { .frame = frame, .class = class, .error = LLFREE_ERR_OK };
}
static inline llfree_result_t ll_unused llfree_err(uint8_t err) {
    return (llfree_result_t) { .frame = frame_id(0), .class = 0, .error = err };
}

/// Check if the result is ok (no error)
static inline bool ll_unused llfree_is_ok(llfree_result_t r) { return r.error == LLFREE_ERR_OK; }
...
static inline llfree_request_t ll_unused llreq(uint8_t order, uint8_t class, ll_optional_t local) {
    return (llfree_request_t) { .order = order, .class = class, .local = local };
}
...
/// This is faster than llfree_stats as it doesn't scan the lower allocator, but may be less
/// accurate.
...
static inline llfree_policy_t ll_unused llfree_simple_policy(uint8_t requested, uint8_t target,
                                                             size_t free) {
    if (requested > target) return (llfree_policy_t) { LLFREE_POLICY_STEAL, 0 };
    if (requested < target) return (llfree_policy_t) { LLFREE_POLICY_DEMOTE, 0 };
    /* same class */
    if (free >= LLFREE_TREE_SIZE / 2) return (llfree_policy_t) { LLFREE_POLICY_MATCH, 1 };
    if (free >= LLFREE_TREE_SIZE / 64) return (llfree_policy_t) { LLFREE_POLICY_MATCH, UINT8_MAX };
...
static inline llfree_classing_t ll_unused llfree_classing_simple(size_t cores) {
    llfree_classing_t t = { .num_classes = 2, .default_class = 1, .policy = llfree_simple_policy };
...
static inline llfree_request_t ll_unused llfree_simple_request(size_t cores, uint8_t order,
                                                               size_t core) {
    if (order >= LLFREE_HUGE_ORDER) return llreq(order, 1, ll_some(core % cores));
...
static inline llfree_policy_t ll_unused llfree_movable_policy(uint8_t requested, uint8_t target,
                                                              size_t free) {
    if (requested > target) return (llfree_policy_t) { LLFREE_POLICY_STEAL, 0 };
    if (requested < target) return (llfree_policy_t) { LLFREE_POLICY_DEMOTE, 0 };
    /* same class */
...
static inline llfree_classing_t ll_unused llfree_classing_movable(size_t cores) {
    llfree_classing_t t = { .num_classes = 3, .default_class = 2, .policy = llfree_movable_policy };
    t.classes[0] = (llfree_class_conf_t) { .class = 0, .count = cores };
    t.classes[1] = (llfree_class_conf_t) { .class = 1, .count = cores };
...
static inline llfree_request_t ll_unused llfree_movable_request(size_t cores, uint8_t order,
                                                                size_t core, bool movable) {
    if (order >= LLFREE_HUGE_ORDER) return llreq(order, 2, ll_some(core % cores));
    if (movable) return llreq(order, 1, ll_some(core % cores));
    return llreq(order, 0, ll_some(core % cores));
}
